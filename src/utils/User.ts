...
export const getWeather = (
  ...
  return new Promise((resolve, reject) => {
    uni.request({
      url: `${weatherApi}${
        now ? "now" : "7d"
      }?key=${mockKey}&location=${longitude},${latitude}`,
      method: "GET",
...
export const getLocationMoreDetail = (
  longitude: number,
  latitude: number
...
export const getWeatherInfo = async (): Promise<WeatherDataType[] | null> => {
  try {
    ...
    // 获得地址信息， 存入数组
    const addressData = isResponseString(
      await getLocationMoreDetail(longitude, latitude)
    );

    // 获得天气信息
    const data = isResponseString(await getWeather(longitude, latitude));
    // 获得当前天气信息
    const nowData = isResponseString(
      await getWeather(longitude, latitude, true)
    );
